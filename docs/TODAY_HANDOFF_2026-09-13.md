...
- 当前 HEAD：`4709122 feat: first quest chain, plus a self-review pass on phase 3`（本日最后一条是阶段 3 单元 3 + 自审）
...
## 今天收尾的一段（第 3 阶段单元 3「首批连续任务」+ 自审）

| 提交 | 内容 |
| --- | --- |
| `4709122` | 首批连续任务（3 条串行主线）+ 对阶段 3 三个单元的一轮自审 |

- 新增 `data/quests.json` + `scripts/systems/QuestSystem.gd`：一次一条、串行接链；**无限时、无失败态、条件全是单调量**（计数/技能/钱/好感/天数）→ 不形成死路。`validate()` 在加载时体检内容（步骤类型写错＝不报错的死路）。进度存 `GameState.quests`，奖励只发一次。
- 计数由 `Game` 在真实结算点喂（上班/做饭/买东西），**只对当前激活任务生效**（刻意，测试已锁；多任务并行时要重审）。
- 任务挂在 HUD 目标行后面（**未加新行**）；步进不弹提示，开场白/收尾才弹。
- 新增 `tools/verify_quests.gd`（6 组 0 失败）、`tools/capture_quest.gd`（2 张截图）。**14 套全员回归全绿**。
- **自审修掉的（都有提交记录可查）**：①`_show_toast` 连弹互相顶掉（阶段 2 就在的真 bug）+ 提示框 320×40 装不下两行；②`_refresh_ui` 每帧新建字典、每帧白跑一次 `negotiate()`；③`objective_text()` 传错类型（GameState 对象 vs 字典）；④`verify_quests` 首跑 11 条 FAIL 全是测试侧（忘手动推进、计数喂错、切图后只等一帧就激活交互层）；⑤截图时序（任务提示下一帧才追加）。
- **阶段 3 代码全部落地，只剩人工试玩验收**；之后进第 4 阶段（扩展地图与内容）。
- 未处理（如实记下）：三处活动结算的样板代码值得抽 `_run_activity()`；任务无支线、无并行；`Rules.gd` 年度经济仍未打通。
...
1. ~~NPC 实体化与交谈~~ / ~~日程与关系反馈~~ / ~~工作技能成长~~ / ~~首批连续任务~~：第 3 阶段代码全部完成。
2. **人工验收第 3 阶段**（下一步）：真实试玩走一遍"上班攒手艺 → 谈薪 → 走完三条任务"，看文案与节奏。顺带把第 2 阶段那项 20–30 分钟完整流程验收一起做了。
3. 第 4 阶段扩展地图：公园、咖啡馆、医院、旧巷逐场景标定碰撞与遮挡，先过出生点/出入口/热点连通检查再开放。
4. 之后是第 5 阶段打磨（动画/音效/存档兼容/性能/Web 构建/长流程回归）。
