...
9. Continuation hardening makes the same regression exercise both 1280×720 and a 1024×720 narrower desktop/Web logical viewport, including horizontal containment checks for the primary/status rows and all four action buttons.
10. A follow-up static audit found that the project uses `window/stretch/mode="canvas_items"`; changing only `Window.size` could therefore shrink the physical window while leaving the HUD's logical `get_visible_rect()` at 1280×720. The regression now changes `content_scale_size` as well and asserts the logical viewport really is 1024×720 before accepting the narrow phase.
...
- `tools/verify_hud_header_layout.gd`
  ...
  - checks the contract at the default 1280×720 logical viewport;
  - then changes both `content_scale_size` and window size to 1024×720, asserts the logical visible rect actually changed, and rechecks horizontal/vertical containment after layout settles.
...
- Read-only inspection of `project.godot` confirmed the project baseline is a 1280×720 logical viewport with `window/stretch/mode="canvas_items"`.
- The prepared narrow regression explicitly rejects a test that merely resizes the physical window: the 1024 phase must first prove `get_visible_rect().size == Vector2(1024, 720)`, then the primary row, status row, HUD envelope and each action button must all fit the logical viewport/HUD envelope.

## Validation
### Performed in this web worker
- Re-read the latest `TASK_BOARD`, `FILE_OWNERSHIP`, `WEB_AGENT_LAUNCHPAD`, current task branch report and branch comparison before this continuation.
- Confirmed UI-FIX-003 is still `READY` on the orchestrator board while this worker report remains `NEEDS_REVIEW`; no new orchestrator review finding or scope expansion is present.
- Confirmed the task branch remains ahead of and not behind `orchestrator/multi-agent-bootstrap` before continuation.
- Read-only inspected `project.godot` and confirmed `viewport_width=1280`, `viewport_height=720`, `window/stretch/mode="canvas_items"`.
- Re-read the narrow regression and corrected the 1024 phase so it exercises logical layout space rather than relying on physical `Window.size` alone.
- Production HUD code was not changed in this continuation.
...
- `tools/verify_hud_header_layout.gd` was **not** executed at either logical viewport size.
...
That single narrow regression checks the default 1280×720 logical viewport and then an explicitly verified 1024×720 logical viewport. Then run the existing location regression to prove the wider location/UI contract still holds:

`godot --headless --path . --script res://tools/verify_locations.gd`

### Required rendered evidence before visual PASS
On the exact review SHA, inspect the running game at the normal 1280×720 target and at 1024×720 (or another narrower common desktop/web logical viewport no wider than that). Confirm all of the following:
...
- Very narrow/mobile-sized viewports are not claimed as supported by this task. The regression defines 1024×720 as one narrower desktop/Web acceptance point rather than inventing a mobile contract.
- Ellipsis is an intentional tradeoff for stable top-band ownership; full long copy remains available through tooltips.
- Actual Godot container minimum-size behavior and button reachability still require the prepared regression/runtime check before final acceptance. The logical-viewport assertion prevents a stretched physical window from giving a false narrow-layout PASS.
