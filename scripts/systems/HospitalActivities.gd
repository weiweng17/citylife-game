...
##
## 结构与咖啡馆/公园/公司一致：距离校验、防重复结算、活动期锁输入；
## 数值结算交给 Game，这里只负责触发、朝向与展示。
##
...
func configure(manager) -> void:
	location = manager
	layer = Control.new()
	layer.name = "HospitalInteractionLayer"
	...
	for id in SPOTS:
		...
		buttons[id] = button
	prompt = Label.new()
	# 底部面板从 y=608 起，提示行放 580（放 600 会被盖住，同公司/便利店）。
...
func _process(_delta: float) -> void:
	...
	if not blocked:
		prompt.text = "消毒水的味道。走近诊桌或候诊椅，按 E 或点击标签" if nearest.is_empty() else "[E] %s  —  %s" % [SPOTS[nearest].label, SPOTS[nearest].detail]
...
## 与其他场景一致；不要 flat=true，否则 normal 底色不会被绘制。
