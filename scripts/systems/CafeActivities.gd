...
##
## 结构与公园/公司/便利店一致：距离校验、防重复结算、活动期锁输入；
...
func configure(manager) -> void:
	location = manager
	layer = Control.new()
	layer.name = "CafeInteractionLayer"
...
func _process(_delta: float) -> void:
	...
	if not blocked:
		prompt.text = "雨声混着磨豆声。走近吧台或圆桌，按 E 或点击标签" if nearest.is_empty() else "[E] %s  —  %s" % [SPOTS[nearest].label, SPOTS[nearest].detail]
