...
## 玩家到了以后没事情可做。
##
## 结构与医院/咖啡馆/公园一致：距离校验、防重复结算、活动期锁输入；
...
func configure(manager) -> void:
	location = manager
	layer = Control.new()
	layer.name = "AlleyInteractionLayer"
...
func _process(_delta: float) -> void:
	...
	if not blocked:
		prompt.text = "巷子里的雨声更近。走近神龛或木门，按 E 或点击标签" if nearest.is_empty() else "[E] %s  —  %s" % [SPOTS[nearest].label, SPOTS[nearest].detail]
