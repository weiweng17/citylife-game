...
## 玩家到了以后没事情可做。
##
## 结构与出租屋/公司/便利店一致：距离校验、防重复结算、活动期锁输入；
...
func configure(manager) -> void:
	location = manager
	layer = Control.new()
	layer.name = "ParkInteractionLayer"
...
func _process(_delta: float) -> void:
	...
	if not blocked:
		prompt.text = "雨还没停。走近长椅或池塘，按 E 或点击标签" if nearest.is_empty() else "[E] %s  —  %s" % [SPOTS[nearest].label, SPOTS[nearest].detail]
