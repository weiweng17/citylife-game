...
##
## 结构与旧巷/医院/咖啡馆一致：距离校验、防重复结算、活动期锁输入；
...
func configure(manager) -> void:
	location = manager
	layer = Control.new()
	layer.name = "RooftopInteractionLayer"
...
func _process(_delta: float) -> void:
	...
	if not blocked:
		prompt.text = "风比楼下凉。走近栏杆或长椅，按 E 或点击标签" if nearest.is_empty() else "[E] %s  —  %s" % [SPOTS[nearest].label, SPOTS[nearest].detail]
