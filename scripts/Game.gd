...
func _refresh_ui() -> void:
	...
	if office_activities:
		...
		var context: Dictionary = {"skill": skill, "wage": wage, "title": JobGrowthScript.title_of(skill), "can_negotiate": JobGrowthScript.can_negotiate(skill), "raised_today": int(game_state.raise_day) == today, "friend": friend}
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if not pressed:
		return
	if (dialog_ui and dialog_ui.is_busy()) or (event_ui and event_ui.is_busy()) or (ending_ui and ending_ui.visible):
		return
...
func _sleep_through_night() -> String:
	...
	_sync_needs_to_time()
	# GAME-FIX-007: 过夜需求结算一旦跨过终局阈值，必须先让统一 evaluator 观察到，
	# 不能被随后同一流程的睡眠恢复“救活”。
...
func _work_growth_line(growth: Dictionary) -> String:
	if bool(growth.get("tier_up", false)):
		return "手上的活终于有了章法——你算得上「%s」了。（时薪 %d）" % [str(growth["tier_title"]), JobGrowthScript.wage_of(skill, game_state.raise_steps)]
...
func _do_negotiate() -> void:
	...
	if not JobGrowthScript.can_negotiate(skill):
		_show_toast("话到嘴边又咽了回去——手上的活还不够硬。（谈薪要技能 %d，你现在 %d）" % [JobGrowthScript.NEGOTIATE_SKILL, skill])
		return
...
func _on_shop_use(item_id: String) -> void:
	...
	if daily_routine and int(effects.get("fullness", 0)) >= MEAL_FULLNESS:
		daily_routine.complete("meal")
	shop_ui.refresh(money, inventory)
	shop_ui.set_status("%s（%s，耗时%d分钟）" % [InventoryScript.use_text(item_id), InventoryScript.effect_text(item_id), minutes])
	_refresh_ui()
...
func _talked_today(npc_id: String) -> bool:
	if npc_relations_sys == null or time_sys == null: return false
...
func _show_event(e) -> void:
	...
	if event_ui:
		...
		event_ui.show_event("%s · %s" % [str(e.get("speaker", "")), str(e.get("title", ""))], str(e.get("text", "")), option_views, background)

func _try_encounter(scene: String):
	if encounter_sys == null or time_sys == null: return null
...
func _show_encounter(e: Dictionary) -> void:
	...
	if event_ui:
		var background: Texture2D = ArtCatalog.event_background(e)
		event_ui.show_event("奇遇 · %s" % str(e.get("title", "")), str(e.get("text", "")), option_views, background)
...
func _fmt_money(v: int) -> String:
	if absi(v) >= 10000: return "%d 万" % int(v / 10000)
	return "%d 元" % v
...
func _show_dialog(speaker: String, lines: Array) -> void:
	...
	if dialog_ui: dialog_ui.show_dialog(speaker, lines)
...
func _evaluate_quests() -> void:
	if quest_sys == null or time_sys == null: return
	var st := _state()
...
func _show_toast(text: String, append: bool = false) -> void:
	...
	if append and toast_label.visible and not toast_label.text.is_empty(): toast_label.text = "%s\n%s" % [toast_label.text, text]
	else: toast_label.text = text
