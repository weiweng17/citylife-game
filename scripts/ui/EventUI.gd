...
func _build_ui() -> void:
	...
	# Dynamic narrative + choice content owns one bounded scroll region. The panel can
	# therefore stay inside its viewport envelope without silently clipping lower choices.
	...
	# Result continuation stays outside the scrolling region so long result copy cannot
	# push the primary continuation action below the panel boundary.
