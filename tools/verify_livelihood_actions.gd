...
func run() -> void:
	...
	assert("2小时" in office_source and "健康−4" in office_source and "心情−8" in office_source, "overtime label must expose time/stat costs")
	...
	assert("90分钟" in cafe_source and "55元" in cafe_source and "健康−2" in cafe_source and "心情−4" in cafe_source, "side-gig label must expose reward/time/stat costs")
